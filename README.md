...
- Supported stack ([add your own](#adding-custom-personas)):
  - 🔸 Next.js
...
4. Adjust the generated template files.

    Here's the bundled Next.js template for reference:
...
5. Deploy the template with the E2B CLI
...
6. Open [lib/templates.ts](lib/templates.ts) in your code editor.

    Add your new template to the list. Here's an example for the bundled Next.js persona:

    ```ts
    [getTemplateIdSuffix('nextjs-developer')]: {
      name: 'Next.js developer',
      lib: ['nextjs@14.2.5', 'typescript', '@types/node'],
      entry: 'pages/index.tsx',
      instructions: 'Pełna aplikacja Next.js 13+ korzystająca z routera Pages.',
      port: 3000,
    }
    ```

    Provide a template id (as key), name, list of dependencies, entry (główna ścieżka pliku) and an optional port. Możesz również dodać dodatkowe instrukcje przekazywane do modelu LLM.

7. Optionally, add a new logo under [public/thirdparty/templates](public/thirdparty/templates)
